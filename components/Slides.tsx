...
export const slides : React.FC<{
  ...
  return (<>
    ...
    <div id="slides-settings" className="settings ml3">
      <div className="f7 o-40 fw5 bb pa1 pb0 pt2" style={{ width: "fit-content"}}>
      ...
      <div className="setting flex flex-row h1 ma1">
        ...
        <input 
            ...
            name="iframe"
            placeholder= ""
            onMouseDown={(event) => { if (!event.target.value) iframe.set(event.target.placeholder) }}
            onMouseUp={(event) => { event.target.select() }}
            onChange={(event) => { iframe.set(event.target.value) }}
            type="text"
      ...
      <p className="f7 gray pa2">
        Paste in a link to any embeddable web page (really any webpage at all), 
        and the slides tool will load it into the iFrame. <br/> <br/>

        Clicking NEXT and PREVIOUS will scroll the frame down/up by a full page height.
      </p>
    </div>
  </>)
